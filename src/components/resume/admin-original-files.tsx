...
export function AdminOriginalFiles({ userId, refreshKey = 0 }: { userId: string; refreshKey?: number }) {
  ...
  return <section id="original-files" className="glass-card rounded-2xl p-6 space-y-4 scroll-mt-6">
    ...
    {files.map(file => <div key={`${file.bucket}/${file.name}`} className="flex flex-wrap items-center gap-3 rounded-xl border border-border p-3">
      ...
      <DeleteResumeButton title={file.name} endpoint={`/api/admin/users/${userId}/files?${new URLSearchParams({ bucket: file.bucket, name: file.name })}`} scope="This permanently removes this original uploaded file. Parsed resumes, separately generated drafts, and downloaded copies remain." onDeleted={() => setFiles(current => current.filter(item => item.bucket !== file.bucket || item.name !== file.name))} />
    </div>)}
  </section>;
}
