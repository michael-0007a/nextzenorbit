...
export function ResumeFileUpload({ file, onChange, disabled, error, maxMB = 5, allowDoc = false }: { file: File | null; onChange: (file: File | null) => void; disabled: boolean; error?: string; maxMB?: number; allowDoc?: boolean }) {
  ...
  function choose(files: FileList | null) {
    ...
    if (!(allowDoc ? /\.(pdf|doc|docx)$/i : /\.(pdf|docx)$/i).test(selected.name)) { setLocalError(`Choose a PDF or Word (${allowDoc ? ".doc, .docx" : ".docx"}) file.`); return; }
    if (!selected.size || selected.size > maxMB * 1024 * 1024) { setLocalError(`Choose a non-empty file up to ${maxMB} MB.`); return; }
    setLocalError(""); onChange(selected);
  }
  return <div className="space-y-2">
    <input ref={input} id="resume-file" name="file" type="file" accept={allowDoc ? ".pdf,.doc,.docx" : ".pdf,.docx"} disabled={disabled} className="sr-only" aria-label="Choose resume" aria-describedby="resume-file-help resume-file-error" onChange={event => choose(event.target.files)} />
    <div onDragOver={event => { event.preventDefault(); if (!disabled) setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); choose(event.dataTransfer.files); }}
      ...
      </div> : <div className="text-center"><UploadCloud className="mx-auto mb-3 h-8 w-8 text-primary" /><p className="font-medium text-sm">Drop your resume here</p><p className="mt-1 text-xs text-text-secondary">PDF or Word document · Up to {maxMB} MB</p></div>}
