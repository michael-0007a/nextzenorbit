  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      try {
        ...
        if (json.success) {
          // If we are searching and the API only searched email, we can do client-side full_name filter too
          let data = json.data;
          if (search) {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-text-secondary">
            Manage users, check profile completeness, and view claim status.
          </p>
        </div>
      </div>
                    ...
                    <td className="px-4 py-3">
                      {user.claimedBy ? (
                        <span className="inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded bg-white/5 text-text-secondary border border-border/40">
                          ...
                          {user.claimedByName || "Claimed"}
                        </span>
                      ) : (
                        <span className="inline-flex items-center gap-1.5 px-2 py-1 text-xs font-medium rounded bg-white/5 text-text-secondary border border-transparent">
                          Unclaimed
                        </span>
                      )}
                    </td>
